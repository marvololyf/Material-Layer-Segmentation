...
# 计算项目根目录 (即 src 的上一级目录)
PROJECT_ROOT = os.path.abspath(os.path.join(SCRIPT_DIR, ".."))


def load_config():
    # 优先在根目录下的 configs/config.json 查找，兼容同级目录查找
...
# 模型路径：如果 config 中为相对路径，则相对于 PROJECT_ROOT 解析
...
FOUR_LAYER_RED_MIN_GAP_RATIO = 0.03
THREE_LAYER_TOP_TOLERANCE_RATIO = 0.025
...
def load_model():
    if not os.path.exists(MODEL_PATH):
        raise FileNotFoundError(
            f"找不到模型权重：\n{MODEL_PATH}\n\n请把 {CONFIG['paths'].get('model_path', 'best_unet_model_v3.pth')} 放在本脚本同一目录。"
        )
...
def get_scale_exclusion_roi(image_rgb):
    ...
    left_ratio = float(SCALE_ROI_LEFT_RATIO)
    top_ratio = float(SCALE_ROI_TOP_RATIO)

    if not 0.0 <= left_ratio < 1.0:
        raise ValueError(
            f"scale_roi_left_ratio 必须位于 [0, 1) 范围内，当前值：{left_ratio}"
        )

    if not 0.0 <= top_ratio < 1.0:
        raise ValueError(
            f"scale_roi_top_ratio 必须位于 [0, 1) 范围内，当前值：{top_ratio}"
        )

    x0 = int(round(w * left_ratio))
...
def mask_out_scale_region(pred_mask, image_rgb):
    """
    返回用于层型判断和边界提取的掩膜。
    比例尺区域使用不会与 0~NUM_CLASSES-1 冲突的类别值填充。
    """
...
def curve_from_layer_top(pred_mask, class_id, roi=None):
    ...
    for x in range(x0, x1):
        col = np.where(pred_mask[y0:y1, x] == class_id)[0]
        if col.size > 0:
            xs.append(x)
...
def get_class_stats(pred_mask, class_id, roi):
    x0, y0, x1, y1 = roi
    roi_mask = pred_mask[y0:y1, x0:x1]
    class_mask = (roi_mask == class_id)
    pixels = int(np.sum(class_mask))

    column_hits = np.any(class_mask, axis=0)
    column_ratio = float(np.mean(column_hits)) if column_hits.size > 0 else 0.0

    return {
        "pixels": pixels,
        "column_ratio": column_ratio,
        "exists": pixels >= MIN_CLASS_PIXELS_IN_ROI and column_ratio >= MIN_CLASS_COLUMN_RATIO,
    }


def classify_mode(pred_mask, image_rgb, roi):
    # 层型判断必须排除右下角比例尺区域，避免比例尺被误判为第 4 层
    ...
    four_rc = curve_from_layer_top(analysis_mask, four_map["ceramic"], roi=roi)
    four_cb = curve_from_layer_top(analysis_mask, four_map["bonding"], roi=roi)
    four_bs = curve_from_layer_top(analysis_mask, four_map["substrate"], roi=roi)

    three_cb = curve_from_layer_top(analysis_mask, three_map["bonding"], roi=roi)
    three_bs = curve_from_layer_top(analysis_mask, three_map["substrate"], roi=roi)
    ...
    if four_ok and three_ok:
        ...
        three_green_gap = np.nanmedian(three_bs[xs] - three_cb[xs])

        if np.isfinite(four_red_gap) and four_red_gap > 6:
            return "four_layer"
        if np.isfinite(three_green_gap) and three_green_gap > 6:
            return "three_layer"
...
def run_tesseract_ocr(image, psm_values=(7, 8, 6, 11)):
    if not os.path.isfile(TESSERACT_CMD):
        raise FileNotFoundError(f"找不到 OCR 程序：{TESSERACT_CMD}")
    if image is None or image.size == 0:
        return ""

    enlarged = cv2.resize(image, None, fx=4, fy=4, interpolation=cv2.INTER_CUBIC)
    if enlarged.ndim == 3:
        enlarged = cv2.cvtColor(enlarged, cv2.COLOR_RGB2GRAY)
    ...
    binary = cv2.copyMakeBorder(
        binary, 18, 18, 24, 24, cv2.BORDER_CONSTANT, value=255
    )
    ...
    try:
        ...
        if not ok:
            raise RuntimeError("比例尺 OCR 临时图像编码失败。")
...
def make_scale_text_regions(scale_crop, bar):
    ...
    for horizontal_pad_ratio, vertical_ratio in (
        ...
        horizontal_pad = max(12, int(round(width * horizontal_pad_ratio)))
        text_height = max(24, int(round(width * vertical_ratio)))
        x0 = max(0, x - horizontal_pad)
        x1 = min(crop_w, x + width + horizontal_pad)
        y0 = max(0, y - text_height)
        y1 = min(crop_h, y + max(2, height // 2))
...
def recognize_scale(image_rgb):
    ...
    attempts = []
    for region in make_scale_text_regions(scale_crop, bar):
        for variant in make_ocr_variants(region):
            text = run_tesseract_ocr(variant)
            attempts.append(text)
            if re.search(r"\d", text):
                scale_value_um = parse_scale_value_um(text)
                return (
                    scale_value_um / bar["pixels"],
                    scale_value_um,
                    bar["pixels"],
                    text,
                )

    for variant in make_ocr_variants(scale_crop):
        text = run_tesseract_ocr(variant, psm_values=(11, 6))
        attempts.append(text)
        if re.search(r"\d", text):
            scale_value_um = parse_scale_value_um(text)
            return (
    ...
    recognized = " | ".join(text for text in attempts if text) or "<空>"
    raise ValueError(f"OCR 未识别出比例尺数字（多方案结果：{recognized!r}）。")
...
def measure_layer_thickness(
    ...
    for key, display_name, upper_curve, lower_curve in layer_specs:
        ...
        # --- 计算逻辑二：逐列全像素采样 ---
        upper_y_f = np.asarray(upper_curve[xs_full], dtype=np.float32)
        lower_y_f = np.asarray(lower_curve[xs_full], dtype=np.float32)
        gaps_px_f = lower_y_f - upper_y_f
        valid_f = np.isfinite(gaps_px_f) & (gaps_px_f > 0)

        full_res = None
        if np.any(valid_f):
            v_xs_f = xs_full[valid_f]
            vals_um_f = gaps_px_f[valid_f] * float(microns_per_pixel)
            full_res = {
                "xs": v_xs_f,
                "upper_y": upper_y_f[valid_f],
                "lower_y": lower_y_f[valid_f],
                "values_um": vals_um_f,
                "min_um": float(np.min(vals_um_f)),
                "max_um": float(np.max(vals_um_f)),
                "mean_um": float(np.mean(vals_um_f)),
                "count": int(vals_um_f.size),
            }
...
class PredictApp:
    ...
    def run_predict(self):
        ...
        try:
            ...
            scale_error = None
            try:
                um_per_pixel, scale_um, scale_pixels, _ = recognize_scale(self.image_rgb)
            ...
            if not found:
                ...
                self.status_var.set("处理完成：已完成分层划线、孔隙率统计和双逻辑厚度测量。")
            elif scale_error:
